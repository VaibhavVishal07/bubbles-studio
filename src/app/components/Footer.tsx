...
function FootLabel({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="uppercase font-bold opacity-60 mt-7 mb-2.5"
...
export default function Footer() {
  ...
  return (
    <footer id="contact" className="px-3 sm:px-4">
      <div
        className="relative overflow-hidden rounded-3xl max-w-[1440px] mx-auto text-white px-[clamp(20px,4vw,56px)] pt-[clamp(80px,12vh,140px)] pb-10 mt-[clamp(60px,10vh,120px)]"
        style={{ backgroundColor: PURPLE }}
      >
        <div ref={ctaRef} className="text-center mb-[clamp(100px,15vh,160px)]">
          <h2
            data-reveal
            style={{
          ...
          <button
            data-reveal
            type="button"
            className="mt-11 rounded-full bg-white px-8 py-4 font-semibold transition-transform duration-300 hover:scale-105"
        ...
        <div
          ref={gridRef}
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_1fr_1.4fr] gap-[clamp(32px,4vw,64px)] pb-[clamp(60px,9vh,110px)] border-b border-white/25"
        >
          <div data-col>
          ...
          <div data-col>
            ...
            >
              <input
                ...
                className="flex-1 min-w-0 rounded-full bg-white/15 border border-white/30 px-5 py-3 placeholder:text-white/60 outline-none focus:border-white/70 transition-colors"
              />
              <button
                type="submit"
                className="rounded-full bg-white px-6 py-3 font-semibold shrink-0 transition-transform duration-300 hover:scale-105"
                style={{ color: PURPLE }}
              >
                Subscribe
              </button>
